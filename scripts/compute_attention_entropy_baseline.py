...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Compute attention-entropy baseline in streaming mode (no large artifact writes)."
    )
    p.add_argument(
        "--splits-dir",
        type=Path,
    ...
    p.add_argument(
        "--layers",
        default="last4",
        help="Layer aggregation: last1, last4, or all",
    )
    p.add_argument(
        "--limit-per-split",
        type=int,
    ...
    p.add_argument(
        "--out-metrics-json",
        type=Path,
        default=Path("outputs/person3_full_logitlens/attention_entropy_metrics.json"),
    )
    return p.parse_args()
...
def _load_split(path: Path, split_name: str, limit: int) -> list[RawSample]:
    rows: list[RawSample] = []
...
def _entropy_from_attention(att: torch.Tensor, eps: float = 1e-12) -> torch.Tensor:
    # att shape: [heads, query_tokens, seq_len]
    p = torch.clamp(att, min=eps)
    return -(p * torch.log(p)).sum(dim=-1)  # [heads, query_tokens]


def _select_layers(n_layers: int, mode: str) -> list[int]:
    if mode == "all":
...
def _compute_sample_attention_entropy(
    tokenizer,
    model,
    sample: RawSample,
    split_name: str,
    ...
    # Keep the tail under truncation so answer tokens remain available.
    ...
    seq_len = int(enc_full["input_ids"].shape[1])
    ...
    with torch.no_grad():
        out = model(**enc_full, output_attentions=True, return_dict=True)

    attentions = out.attentions
    if not attentions or len(attentions) == 0:
        return float("nan"), ans_end - ans_start
    layer_ids = _select_layers(len(attentions), layer_mode)

    layer_scores: list[float] = []
    for li in layer_ids:
        # [1, heads, seq, seq] -> [heads, answer_tokens, seq]
        att = attentions[li][0, :, ans_start:ans_end, :].detach().float().cpu()
        if att.numel() == 0:
            continue
        ent = _entropy_from_attention(att)
        layer_scores.append(float(ent.mean().item()))

    if not layer_scores:
        return float("nan"), ans_end - ans_start
    return float(np.mean(layer_scores)), ans_end - ans_start
...
def main() -> None:
    ...
    for split_name in ["train", "test"]:
        ...
        rows = _load_split(split_file, split_name=split_name, limit=args.limit_per_split)
        print(f"Processing {split_name}: {len(rows)} samples")

        for i, rs in enumerate(rows, start=1):
            score, ans_tokens = _compute_sample_attention_entropy(
                tokenizer=tokenizer,
                model=model,
                sample=rs,
                split_name=split_name,
                max_seq_len=args.max_seq_len,
                layer_mode=args.layers,
                device=device,
            )
            ...
            all_rows.append(
                {
                    ...
                    "split": split_name,
                    "label": y,
                    "attention_entropy": score,
                    "answer_token_count_used": ans_tokens,
                }
            )
    ...
    payload = {"metadata": meta, "attention_entropy_baseline": metrics}
